/*
东东萌宠选择商品
cron 25 9,13 * * * jd_petAutoChoiceGoods.js
*/
    ...
    try {
        ...
        if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
            ...
            if ($.petInfo.userStatus === 0) {
                console.log('萌宠活动未开启，尝试开启活动.')
                await goodsInfoUpdate()
                await newUserGifts();
            } else if (!$.petInfo.goodsInfo || $.petInfo.petStatus === 6) {
                console.log(`已领取红包,但未选购新的商品...`)
                await goodsInfoUpdate()
            } else if ($.petInfo.petStatus === 5) {
                console.log(`可以兑换红包...`)
                await redPacketExchange()
                await goodsInfoUpdate()
            } else {
...
async function redPacketExchange () {
    const functionId = arguments.callee.name.toString();
    const res = await request(functionId);

    if (res.code === '0' && res.resultCode === '0') {
        message += `成功兑换红包${res.result.discount}元\n`
    } else {
        message += '兑换红包失败！\n'
        console.log(`兑换红包失败: ${JSON.stringify(res)}`)
    }
}


async function goodsInfoList () {
    const functionId = arguments.callee.name.toString();
    const res = await request(functionId, { type: 2 });
    if (res.code === '0' && res.resultCode === '0') {
        console.log('成功获取')
...
async function newUserGifts () {
    ...
    if (res.code === '0' && res.resultCode === '0') {
        ...
        message += '开启东东萌宠失败！\n请手动去京东APP开启活动\n入口：我的->游戏与互动->查看更多开启'
        console.log(`开启东东萌宠失败: ${JSON.stringify(res)}`)
    }
}

async function goodsInfoUpdate () {
    const goodsList = await goodsInfoList();
    if (goodsList && goodsList.length > 0) {
        const { goodsId, goodsName } = anyOne(goodsList);
        const functionId = arguments.callee.name.toString();
        const res = await request(functionId, { goodsId: goodsId });
        if (res.code === '0' && res.resultCode === '0') {
            message += `成功领养商品: ${goodsName}\n`
        } else {
            message += `领养商品失败！\n`
            console.log(`领养商品失败：${JSON.stringify(res)}`)
        }
    }
}
