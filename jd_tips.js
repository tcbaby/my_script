...
function getCityPinLog () {
    ...
    const pinMap = getLogMap('jd_city');
    Object.keys(pinMap).forEach(pin => {
        if (!pinMap[pin] || pinMap[pin].indexOf('账号已黑') != -1) {
            delete pinMap[pin]
        } else {
            pinMap[pin] = pinMap[pin].split('\n').filter(line => !line.match(/{.*}/)).join('\n')
            let idx = pinMap[pin].indexOf('好友互助码】')
            if (idx != -1) {
                pinMap[pin] = 'code：' + pinMap[pin].substring(idx + 6);
            }
        }
    })
    return pinMap;
...
function getSignFeePinLog () {
    ...
    const pinMap = getLogMap('jd_signFree');
    Object.keys(pinMap).forEach(pin => {
        if (!pinMap[pin] || pinMap[pin].indexOf('没有需要签到的商品') != -1) {
            delete pinMap[pin]
        }
    })
    return pinMap;
...
/**
 * 
 * @returns key: pin, value: 对应日志
 */
function getLogMap (scriptName) {
    const file = getLastLogFile(scriptName);
    const map = {};
    ...
    if (file) {
        ...
        for (let i = idxs[idxs.length]; i < lines.length; ++i) {
            if (lines.indexOf('通知') != -1 || lines.indexOf('结束') != -1 || i + 1 == lines.length) {
                idxs.push(i);
            }
        }

        for (let i = 0; i < idxs.length; ++i) {
            let start = idxs[i], end = idxs[i + 1];
            console.log(lines[start])
            let pin = lines[start].match(/.*开始【京东账号.*?】(.*?)\*/)[1]
            if (pin) {
                map[pin] = lines.filter((l, idx) => idx > start && idx < end).join('\n').trim();
            }
        }
    }
    return map;
...
function requireConfig () {
    if (process.env.JD_COOKIE && process.env.TIPS_LOG_PATH_PRIFIX) {
        ...
        if (fs.existsSync(tipsFile)) {
            const data = fs.readFileSync(tipsFile, { encoding: 'utf-8' }) || '';
            if (data.trim()) {
                RemainMessage += "\n⭕提醒:⭕\n";
