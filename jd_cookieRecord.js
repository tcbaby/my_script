/*
京东Cookie录入情况： 用于毛估京东Cookie是否过期
京东Cookie录入情况
cron 59 23 * * * jd_cookieRecord.js
*/
const $ = new Env('京东Cookie录入情况');
const fs = require('fs')
...
!(async () => {
    let recordFile = './.jdCookieRecord.json'
    let ckMap = {};
    if (fs.existsSync(recordFile)) {
        let data = fs.readFileSync(recordFile, {encoding: 'utf-8'});
        ckMap = data ? JSON.parse(data) : {};
    }
    console.log(ckMap)
    let date = new Date();
